...
class ChatViewModel @Inject constructor(
...
) : AndroidViewModel(mApplication), SafeApiCall, WebSocketInterface {
    ...
    init {
        ...
        Log.d("checkUserId", mApplication.applicationContext.getUserId(mApplication).toString())
    }
    ...
    var isSimplifyPathPressed by mutableStateOf(false)
            ...
            is PlainMessageRequest -> {
                ...
                safeApiCall {
                    api.sendPlainMessage(data).convertToMessage().copy(receiverId = userId).also {
                        ...
                        isImageBoxEnabled = it.message.contains("Please enter your query")
                    }
                }
            }
            ...
            is InteractiveMessageRequest -> {
                ...
                safeApiCall {
                    api.sendInteractiveMessage(data).convertToMessage().copy(receiverId = userId)
                        .also {
                            isImageBoxEnabled = it.message.contains("Please enter your query")
                            isSimplifyPathPressed = it.message.contains("You can now continue with Simplify Path!")
            ...
            when (response) {
                ...
                is Resource.Success -> {
                    ...
                    if (!messageList.isNullOrEmpty()) {
                        ...
                        if (lastmsg.value.message.contains("Please select valid skills from the dropdown")) {
    ...
    override fun connectSocket(socketUrl: String) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                easyWs = OkHttpClient().easyWebSocket(socketUrl, mApplication.applicationContext)
