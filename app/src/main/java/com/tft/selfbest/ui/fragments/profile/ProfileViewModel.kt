...
//                profileLiveData.postValue()
    ...
    fun updateRegistrationToken(token: String, isActive: Boolean){
        viewModelScope.launch {
            val id = preference.getLoginData?.id?:return@launch
            repository.updateRegistrationToken(token, id, isActive).collect{
                if(it is NetworkResponse.Success)
                    notificationResponse.postValue(it)
            }
        }
    ...
    fun deleteNotificationToken(token: String){
        viewModelScope.launch {
            val id = preference.getLoginData?.id?:return@launch
            Log.e("user-device", "view model")
            repository.deleteNotificationToken(token, id).collect{
                Log.e("Delete Request", "$it")
                if(it is NetworkResponse.Success)
                    notificationResponse.postValue(it)
                else if(it is NetworkResponse.Error)
                    notificationResponse.postValue(it)
