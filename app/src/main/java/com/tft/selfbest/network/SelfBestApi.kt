...
interface SelfBestApi {
    ...
    @POST("microsoft/auth")
    suspend fun loginViaMicrosoft(
        @Query("location") location: String,
        @Body msLogin: MSLogin
    ): LogedInData
...
//    ): List<QueryResponse>
    ...
    @PATCH("https://webbot.self.best/userresponses/user-device-token/")
    suspend fun updateRegistrationToken(
        @Body request: DeviceTokenRequest
    ): DeviceTokenResponse

    @HTTP(method = "DELETE", path = "https://webbot.self.best/userresponses/user-device-token/", hasBody = true)
