...
import com.tft.selfbest.models.MSLogin
...
class LoginRepository @Inject constructor(private val networkClient: SelfBestApiClient) {
    ...
    suspend fun loginViaMicrosoft(idtoken: String, type: String, location: String, reactivate: Boolean) = flow {
        emit(NetworkResponse.Loading())
        emit(NetworkRequest.process { networkClient.apis.loginViaMicrosoft(location, MSLogin(idtoken, type, reactivate)) })
    }.flowOn(Dispatchers.IO)
